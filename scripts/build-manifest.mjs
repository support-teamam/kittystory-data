#!/usr/bin/env node
/**
 * Rebuilds data/manifest.json from data/kitty-tasks.json.
 *
...
const DATA_PATH = join(ROOT, 'data', 'kitty-tasks.json');
const MANIFEST_PATH = join(ROOT, 'data', 'manifest.json');

// Public Pages URL where the app fetches the data from.
const DATA_URL = 'https://support-teamam.github.io/kittystory-data/data/kitty-tasks.json';
...
try {
  ...
  console.error('kitty-tasks.json is not valid JSON:', e.message);
  process.exit(1);
}
